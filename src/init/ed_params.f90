...
subroutine init_can_rad_params()
   ...
   !---------------------------------------------------------------------------------------!
   !----- Visible (PAR). ------------------------------------------------------------------!
   leaf_scatter_vis(1:20) = leaf_reflect_vis(1:20) + leaf_trans_vis(1:20)
   wood_scatter_vis(1:20) = wood_reflect_vis(1:20) + wood_trans_vis(1:20)
   !----- Near infrared. ------------------------------------------------------------------!
   leaf_scatter_nir(1:20) = leaf_reflect_nir(1:20) + leaf_trans_nir(1:20)
   wood_scatter_nir(1:20) = wood_reflect_nir(1:20) + wood_trans_nir(1:20)
   !---------------------------------------------------------------------------------------!
   ...
   !---------------------------------------------------------------------------------------!
   !----- Visible (PAR). ------------------------------------------------------------------!
   leaf_backscatter_vis(1:20) = (2.d0 * leaf_reflect_vis(1:20) - leaf_trans_vis(1:20))     &
                              / (3.d0 * leaf_scatter_vis(1:20))
   wood_backscatter_vis(1:20) = (2.d0 * wood_reflect_vis(1:20) - wood_trans_vis(1:20))     &
                              / (3.d0 * wood_scatter_vis(1:20))
   !----- Near infrared. ------------------------------------------------------------------!
   leaf_backscatter_nir(1:20) = (2.d0 * leaf_reflect_nir(1:20) - leaf_trans_nir(1:20))     &
                              / (3.d0 * leaf_scatter_nir(1:20))
   wood_backscatter_nir(1:20) = (2.d0 * wood_reflect_nir(1:20) - wood_trans_nir(1:20))     &
                              / (3.d0 * wood_scatter_nir(1:20))
   !---------------------------------------------------------------------------------------!
...
subroutine init_pft_photo_params()
   ...
   !---------------------------------------------------------------------------------------!
   Vm_decay_e(1:20)          = 0.4     !                                          [    ---]
   Vm_decay_a(1:20)          = 220000. !                                          [  J/mol]
   Vm_decay_b(1:20)          = 690.    !                                          [J/mol/K]
   !---------------------------------------------------------------------------------------!
   ...
   Rd_low_temp (1:20) = Vm_low_temp (1:20)
   Rd_high_temp(1:20) = Vm_high_temp(1:20)
   Rd_decay_e  (1:20) = Vm_decay_e  (1:20)
   Rd_hor      (1:20) = Vm_hor      (1:20)
   Rd_q10      (1:20) = Vm_q10      (1:20)
   ...
   case (0,1)
      ...
      Rd0         (1:20) = dark_respiration_factor(1:20) * Vm0(1:20)
   ...
   !---------------------------------------------------------------------------------------!
   water_conductance(1:20) = 450. / yr_sec ! 150. / yr_sec * kfact
   !---------------------------------------------------------------------------------------!
...
!   root_turnover_rate(4)          = onethird
   ...
   root_turnover_rate(2)          = 6.6
   root_turnover_rate(3)          = 6.6
   root_turnover_rate(4)          = 6.6
   ...
   root_respiration_factor(1:20)  = 0.528
...
!   q(4)     = 1.0
   ...
   rlength(1:20)  = rdiamet(1:20)**twothirds
...
use pft_coms, only: c2n_leaf, Vm0, SLA, &
     ...
     C2B,plant_N_supply_scale, c2p_alive, c2p_dead, c2p_storage
...
! Set all PFTs to the same C:P ratio for now.
c2p_alive(:) = 100.
c2p_dead(:) = 1000.
c2p_storage(:) = 1000.
...
subroutine init_pft_leaf_params()
   ...
   !---------------------------------------------------------------------------------------!
   c_grn_leaf_dry(1:20)      = 3218.0    ! Jones 1992  J/(kg K)
   c_ngrn_biom_dry(1:20)     = 1256.0    ! Forest Products Laboratory 
   wat_dry_ratio_grn(1:20)   = 2.5       ! 
   !wat_dry_ratio_grn(1:20)   = 1.5       ! Ceccato et al. 2001
   wat_dry_ratio_ngrn(1:20)  = 0.7       ! Forest Products Laboratory
   !---------------------------------------------------------------------------------------!
   ...
   ! assumption.                                                                           !
   !---------------------------------------------------------------------------------------!
   delta_c(1:20) = 100. * wat_dry_ratio_ngrn(1:20)                                         &
                 * (-0.06191 + 2.36e-4 * t3ple - 1.33e-2 * wat_dry_ratio_ngrn(1:20))
...
subroutine init_pft_derived_params()
   ...
   use allometry            , only : h2dbh                & ! function
                                   ...
                                   , dbh2bl               & ! function
                                   , dbh2bd               ! ! function
   use pft_coms, only: c2p_recruit, c2p_alive, c2p_dead
   implicit none
   !----- Local variables. ----------------------------------------------------------------!
   integer                           :: ipft
   real                              :: dbh
   ...
   do ipft = 1,n_pft
      ...
      ! Carbon to phosphorus for recruits.
      c2p_recruit(ipft)      = (balive_min + bdead_min)                                    &
                             / (balive_min * (f_labile(ipft) / c2p_alive(ipft)             &
                             + (1.0 - f_labile(ipft)) / c2p_dead(ipft))  &
                             + bdead_min/c2p_dead(ipft))
      !------------------------------------------------------------------------------------! 
