...
subroutine structural_growth(cgrid, month)
   ...
   use pft_coms, only: c2p_alive, c2p_storage, c2p_recruit, c2p_dead
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
   ...
   real                          :: mort_litter
   real                          :: seed_litter
   real                          :: net_seed_N_uptake, net_seed_P_uptake
   real                          :: net_stem_N_uptake, net_stem_P_uptake
   real                          :: cb_act
   real                          :: cb_max
   real                          :: old_leaf_hcap
   real                          :: old_wood_hcap
...
!stop
               ...
               csite%fsp_in(ipa) = csite%fsp_in(ipa)                                       &
                                 + f_labile(ipft) * balive_mort_litter / c2p_alive(ipft)    &
                                 + bstorage_mort_litter/ c2p_storage(ipft)                       &
                                 + seed_litter / c2p_recruit(ipft)
               csite%stsp_in(ipa) = csite%stsp_in(ipa) + (struct_litter                       &
                                 + (1.0 - f_labile(ipft)) * balive_mort_litter)/c2p_dead(ipft)
               ...
               csite%total_plant_P_uptake(ipa) =                                    &
                      csite%total_plant_P_uptake(ipa) + net_seed_P_uptake           &
                    + net_stem_P_uptake
...
subroutine structural_growth_eq_0(cgrid, month)
   ...
   use pft_coms, only: c2p_storage, c2p_alive, c2p_recruit, c2p_dead
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
   type(edtype)     , target     :: cgrid
   integer          , intent(in) :: month
